...
public class UpgradeManager : MonoBehaviour
{
    ...
    [Header("環繞劍狀態")]
    public float orbitalLifestealPercent = 0f; // 改成趴數，0.05 代表 5%

    [Header("系統設定")]
    public int maxUpgradeLevel = 5; // 每個強化技能的最高等級

    [Header("全域加成")]
    public float globalDamageMultiplier = 1f; // 新增這行：基礎傷害倍率
    ...
    void Start()
    {
        ...
        {
            globalDamageMultiplier = 1.5f; // 第二關：傷害變 1.5 倍
        ...
        {
            globalDamageMultiplier = 2.5f; // 第三關：傷害變 2.5 倍
        ...
        {
            globalDamageMultiplier = 4.0f; // 第四關：傷害變 4 倍 (環繞劍直接變 100 傷！)
            flyingSwordCountPerShot += 2;
            flyingSwordBounceCount += 2;
            if (player != null)
            {
    ...
    void ApplyUpgrade(string upgradeName)
    {
        ...
        switch (upgradeName)
        {
            ...
            case "增加環繞劍吸血":
                orbitalLifestealLv++;
                orbitalLifestealPercent += 0.03f; // 每升 1 級，吸血增加 3% (滿級 15%)
                break;
